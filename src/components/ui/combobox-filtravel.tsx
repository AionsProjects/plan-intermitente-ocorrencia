...
export function ComboboxFiltravel({
  ...
  return (
    <div className="relative" ref={wrapperRef}>
      {label && (
        <label className="mb-2 block text-[10px] uppercase tracking-[0.3em] text-white/55">
          {label}
        </label>
      )}
      ...
      <button
        ...
        disabled={disabled}
        className={`flex w-full items-center justify-between gap-3 rounded-2xl border border-white/12 bg-white/[0.04] px-4 py-3 text-left text-sm transition hover:border-white/22 hover:bg-white/[0.06] ${
          aberto ? "border-white/30 bg-white/[0.08]" : ""
      ...
      >
        <span className={`truncate ${valor ? "text-white" : "text-white/40"}`}>
          {valor || placeholder}
        </span>
        <ChevronDown
          className={`size-4 shrink-0 text-white/55 transition-transform ${aberto ? "rotate-180" : ""}`}
        />
      </button>

      {aberto && (
        <div className="absolute left-0 right-0 z-40 mt-2 overflow-hidden rounded-2xl border border-white/12 bg-[#0a1224]/96 shadow-[0_22px_46px_-16px_rgba(0,0,0,0.7)] backdrop-blur-xl">
          {!semOpcoes && (
            <div className="flex items-center gap-2 border-b border-white/10 px-3 py-2.5">
              <Search className="size-3.5 shrink-0 text-white/55" />
              <input
                ref={inputRef}
                type="text"
                value={busca}
              ...
              {busca && (
                <button
                  type="button"
                  onClick={() => setBusca("")}
                  className="inline-flex size-6 items-center justify-center rounded-full text-white/45 transition hover:bg-white/[0.08] hover:text-white/85"
          ...
          {!semOpcoes && (
            <div className="flex items-center justify-between gap-3 border-b border-white/[0.06] px-4 py-1.5 text-[10px] uppercase tracking-wider text-white/45">
              <span>
          ...
          {semOpcoes ? (
            <p className="px-5 py-6 text-center text-sm text-white/55">
              {emptyMessage}
            </p>
          ) : semResultado ? (
            <p className="px-5 py-6 text-center text-sm text-white/55">
                ...
                return (
                  <li key={o}>
                    <button
                      type="button"
                      onClick={() => escolher(o)}
                      className="flex w-full items-center gap-2 px-4 py-2 text-left text-sm transition hover:bg-white/[0.06]"
                      style={
                        sel
                          ? {
