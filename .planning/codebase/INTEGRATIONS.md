...
- **Not found in this repo** (surfaced only in operator memory, unverified here): a Postgres schema named `nocturnalgoose` reportedly backs the n8n installation's own execution/workflow-definition storage on the *same* shared Postgres cluster addressed by `DATABASE_URL`, queryable directly via SQL without an n8n API key. No code in this repo references `nocturnalgoose` — treat as external/operational knowledge to confirm with infra, not as a documented dependency of this codebase.
...
- **Not found in this repo** (per operator memory, unverified here): the same Postgres cluster is also said to host a `nocturnalgoose` schema belonging to the n8n installation itself (workflow executions/definitions), reachable by direct SQL. No code, migration, or connection string in this repo targets that schema — if it's used operationally (e.g. ad-hoc log queries), it happens outside this codebase.
