...
- **Unidades por contrato: fonte única = RM** (SQL `231375`, via `unidadesRm()` no auth-backend, `auth-backend/src/routes/rmLookups.ts`). Consumidores: `/convocar` (`GET /api/convocar/opcoes`), ponto facultativo e agora **atestados** via hook compartilhado `src/lib/useUnidadesRm.ts` (`GET /api/intermitente-unidades-rm`, react-query staleTime 5min, fallback hardcoded `UNIDADES_POR_CONTRATO` só se RM cair).
- **Contrato ADMINISTRATIVO liberado no /convocar**: `contratoPorCodigo()` ganhou prefixo `01.01.0007.04` → ADMINISTRATIVO (unidade única "ADMINISTRAÇÃO - INTERMITENTES"). Novo `CONTRATOS_RM` (= `CONTRATOS_PF` + ADMINISTRATIVO) usado só no `unidadesRm()` — `CONTRATOS_PF` intacto, ponto facultativo NÃO ganhou ADMINISTRATIVO. Fallback front (`src/lib/unidadesContrato.ts`) atualizado; "ADMINISTRAÇÃO - INTERMITENTES" saiu de `UNIDADES_FORA_ESCOPO`. Restante do `01.01.0007.*` (setores internos CLT) segue fora de propósito.
- **Atestados (WizardDocumento)**: etapa "qual unidade?" lista unidades EXATAS do RM por contrato (+ sentinela `UNIDADE NÃO ENCONTRADA`), substituindo a lista hardcoded de `opcoesAtestadoForm`/lib compartilhada (que vira fallback). Quando a lista do RM chega, pré-seleciona a unidade do empregado (`localUnidade`/`secao`) por match exato normalizado (acento/caixa) — só se o operador ainda não escolheu.
- **Labels de dropdown no Monday criam sozinhas**: create do /convocar (`auth-backend/src/monday.ts`) e WF n8n de atestados (`kVpn69JFUJfR7T7U`) usam `create_labels_if_missing: true` — unidade nova do RM vira label no board no primeiro uso.
- **Pendências relacionadas**: URUGUAIANA tem label no board ENTRADA mas nenhum prefixo RM mapeado (no RM, "IFFAR - URUGUAIANA" vive sob `01.01.0042`) — mesmo fix de 1 linha se precisarem convocar. ADMINISTRATIVO não existe como label da coluna `department` do board Controle de Atestados — se quiserem em atestados, criar label no Monday + adicionar em `CONTRATOS_COLABORADOR`.

## Estado anterior (2026-07-13)

- **Liquid Glass v2 ("vidro fumê meia-noite")** — skin do handoff claude.ai/design "Design system coesão profissional". Tokens+classes em `src/liquid-glass-v2.css` (importado pelo `index.css`; classes em `@layer components` pra utilities Tailwind poderem sobrescrever): `--panel`/`--cta-hi/lo/ink` por esquema (dark+light), `.glass-panel` (moldura, raio 28, blur 32, sheen `::before`), `.glass-hero` (halo do acento), `.glass-tile-v2`, `.glass-field` (input pill afundado; variante `--display` = serif 20px), `.glass-chip`, `.glass-cta` (CTA pill luminoso; variante `--mini`), `.pill-soft`, `.seg-pill`, `.icon-orb` (`--neutral`/`--ring`), `.eyebrow`, `.tilt-3d` (`--suave`). Vocabulário v1 (`.glass-strong`/`.glass-tile`/`.liquid-*`) segue vivo nas telas internas (wizard preencher etc.) — migração gradual.
- **ZoomTransition** (`src/components/ZoomTransition.tsx`) — assinatura de navegação: clicar num tile do hub expande o botão até virar a tela (660ms, label serif crescendo no vidro); Home do NavCluster contrai de volta ao tile de origem. `ZoomProvider` envolve o App; navegação por zoom marca flag módulo-level que o `PageTransition` consome pra pular o slide (`SlideStack` ganhou direction `"none"` = troca instantânea). Respeita prefers-reduced-motion/`data-reduce-anim` (vira navegação instantânea).
- **Telas reskinadas pro v2**: Hub (hero com halo + 4 tiles icon-orb + badge pill neutra + CTA "Começar ↗"), Login (seg-pill Email|CPF, glass-fields pill, CTA luminoso, Google pill-soft), Corrigir (glass-field--display + glass-cta "Abrir"), Convocar/BuscarEmpregado (busca serif + tiles de resultado), Atestados/EscolhaTipoTrabalhador (orbs accent/violeta). Containers de Convocar/Atestados/Ponto/Mensal/Teste/CompletarCadastro trocados de `.glass-strong` pra `.glass-panel`.
- `.claude/launch.json` ganhou config `frontend-alt` (porta 5175, strictPort) pra dev quando a 5174 está ocupada.
- **Acompanhamento do mensal reskinado v2** (`src/features/mensal/Acompanhamento.tsx`, extraído do MensalPage): anel de progresso SVG central (serif, glow do acento; verde/vermelho no fim), contrato atual em glass-tile com trilho das 12 etapas, concluídos com `.lamp--green/--red`, fila como chips fantasma, timeline estilo console mono em poço de vidro, Interromper discreto, CTA "Concluir" luminoso. **Demo local sem envio**: `/mensal?demo=1` (`demoRun.ts`) simula a execução contrato a contrato (TRE PB falha de propósito) sem backend/localStorage. Lamps v2 (`.lamp--green/--yellow/--red/--off`) adicionadas ao `liquid-glass-v2.css`.

## Estado anterior (2026-06-15)
...
- **WF3 Finalizar**: continua responsavel por falta/atraso/desconsiderado do registro manual. Valores de VR/VT vÃªm do board de parametros (`18413870370`). Ao concluir/corrigir, tambem espelha no item origem do Plan: `numeric` = qtd. faltas e `texto5` = minutos totais de atraso, sempre sobrescrevendo o total calculado.
...
- **WF5 Pontual FIFO** â€” convoca pontual: calcula benefÃ­cio do perÃ­odo, abate descontos pendentes FIFO no board Desconto, gera order Caju (crÃ©dito + boleto PIX), SOAPs RM, cria item SolicitaÃ§Ã£o Pagamento (board `18393673859`), espelha valores/dias/crÃ©ditos no item original do Plan e envia arquivos Caju ao Drive (`CAJU/BOLETOS` e `CAJU/COMPROVANTES`) via `POST /drive-intermitente-arquivar`. Documentado completo no `Mapeamento.md`.
- **WF Drive Intermitente** â€” `POST /drive-intermitente-arquivar` no n8n novo. Cria/reaproveita pasta pessoa + perÃ­odo, atualiza links no Plan e na SolicitaÃ§Ã£o de Pagamento. Quando recebe `gerar_planilha_conferencia=true`, chama `POST /gerar-planilha-conferencia` com `item_entrada_id` + pasta correta.
- **WF Gerar Planilha Conferencia** â€” `POST /gerar-planilha-conferencia` no n8n novo. Agora valida `item_entrada_id` e `pasta_convocacao_drive_id`; se vier payload incompleto, responde erro e nÃ£o sobe XLSX na raiz.
...
### Deploy
- **Vercel** — projeto `plan-intermitente-ocorrencia` (team aions). Produção: `https://plan-intermitente-ocorrencia.vercel.app`. Deploy: `vercel deploy` (preview) / `vercel deploy --prod`, ou push via GitHub.
- A VM antiga (`192.168.0.41:8081`, Docker+nginx) foi DESATIVADA — qualquer referência a ela é histórica. Automations do Monday que geravam links pra VM devem apontar pro domínio Vercel.
...
2. **WF Monday Automation** "Gerar Link": ao ativar trigger, gera UUID + link `https://plan-intermitente-ocorrencia.vercel.app/descontos/<uuid>` + Status=Pendente.
...
- **Hub principal em `/`** â€” substitui DevIndex antigo. Operacional bate em `https://plan-intermitente-ocorrencia.vercel.app` e vÃª opÃ§Ãµes claras (Convocar / Corrigir). Mocks acessÃ­veis via `/teste` (entrada discreta).
...
| Faltas registradas | `numeric` | numbers | Espelho do WF3: `qtd_faltas` final do registro manual. Sobrescreve em correcoes; nao incrementa |
| Minutos de atraso registrados | `texto5` | text | Espelho do WF3: total de minutos de atraso. Sobrescreve em correcoes; nao incrementa |
